...
from executor.constants import CHECKPOINT_INTERVAL
from executor.models.task import TaskState, TaskStateAdapter
    ...
    def _remove_tmp(self, tmp_path: str) -> None:
        if os.path.exists(tmp_path):
            try:
                ...
                logger.warning(
                    "Failed to remove state file %s: %s", self.state_path, e
                )
        ...
        while not self._stop_event.is_set():
            ...
            if self._state is not None:
                try:
                    ...
                    logger.warning("State write failed, will retry next cycle")
